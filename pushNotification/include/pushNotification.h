#ifndef _PUSH_NOTIFICATION_H_
#define _PUSH_NOTIFICATION_H_ 1
...
// int cSSL::Hex2Chr(char *DstStr,char *HexStr,int HexLen)
...
bool ConnectSSL(const char* pPassword, const char* appId, const char* pCmd, int32_t nCmdLen)
{
	char szURL[200]   = {'\0'};
	snprintf(szURL, sizeof(szURL), "%s:%d", SERVER_URL, SERVER_PORT);

	int baseLength = strlen("../cert/") + strlen(appId) + 1;

	char pem[baseLength + strlen("/push.pem")];
	char key[baseLength + strlen("/siyao,key")];
	
	snprintf(pem, sizeof(pem), "%s%s%s", "../cert/",appId,"/push.pem");
	snprintf(key, sizeof(key), "%s%s%s", "../cert/",appId,"/siyao.key");
	...
	/** 验证证书文件 */
	if (SSL_CTX_use_certificate_file(m_pSSLCtx, pem, SSL_FILETYPE_PEM) <= 0)
	{
	...
	/** 验证密码文件 */
	BIO* pKeyBuff = BIO_new_file(key, "r");
	if(NULL == pKeyBuff)
	{
...
int main(int argc,char *argv[])
{
	...
	char DeviceToken[] = "7d2f25b619fe77081ac17d26072fb14792ce6879dabd00267ea6def1571822c4";
